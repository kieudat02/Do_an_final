...
// Lấy chi tiết tour theo slug (SEO-friendly)
export const getTourDetailBySlug = (slug) => {
  return axiosInstance.get(API_ENDPOINTS.TOUR_DETAIL_BY_SLUG(slug));
};

// Lấy slug từ ID (để redirect)
export const getTourSlugById = (id) => {
  return axiosInstance.get(API_ENDPOINTS.TOUR_SLUG_BY_ID(id));
};
...
// Tăng views cho tour
export const incrementTourViews = (tourId) => {
  return axiosInstance.post(`/api/public/tours/${tourId}/increment-views`);
};