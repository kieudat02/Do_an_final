...
// Utility function để viết hoa chữ đầu
const capitalizeFirstLetter = (str) => {
  if (!str) return str;
  return str.charAt(0).toUpperCase() + str.slice(1);
};

// Utility function để viết hoa tất cả các từ
...
const TourList = () => {
  ...
  const fetchTours = async (page = 1, isLoadMore = false, isCategoryFilter = false) => {
    try {
      ...
      let data, pagination, category, categoryInfo;
      
      if (slug) {
        // getToursBySlug returns: { success: true, tours: [...], pagination: {...}, categoryInfo: {...} }
        const responseData = response.data;
        data = responseData.tours || [];
        pagination = responseData.pagination || {};
        category = responseData.category || null;
        categoryInfo = responseData.categoryInfo || null;
      } else {
        // getTours returns: { success: true, data: [...], pagination: {...} }
        const responseData = response.data;
      ...
      setTours((prevTours) => (isLoadMore ? [...prevTours, ...toursArray] : toursArray));
      setCurrentPage(page);
      ...
      // Set empty tours array to prevent undefined errors
      setTours([]);
      setCurrentPage(1);
      setHasMoreTours(false);
      setCurrentCategory(null);
      
      // Only redirect to 404 for 404 errors, not for other errors
  ...
  // Cập nhật tour khi search query thay đổi
  useEffect(() => {
    if (searchQuery) {
      // Reset về trang đầu khi có search query mới
      fetchTours(1, false, true);
    }
  }, [searchQuery]);
  ...
  useEffect(() => {
    if (searchQuery) {
      // Khi có search query, hiển thị breadcrumb cho tìm kiếm
      setBreadcrumbData({
        categoryName: `Tìm kiếm: "${searchQuery}"`,
        categorySlug: null,
        tourTitle: null,
        customItems: [
          { name: 'Trang chủ', link: '/' },
          { name: 'Tours', link: '/tours' },
          { name: `Tìm kiếm: "${searchQuery}"`, link: null }
        ]
      });
    } else if (slug) {
      if (currentCategory) {
    ...
    // Không set breadcrumb khi không có slug và không có search - để Tours page handle
  }, [currentCategory, slug, searchQuery, setBreadcrumbData]);
  ...
  const handleSubmitPhone = async () => {
    ...
    try {
      // Tạo form data với thông tin tối thiểu
      const contactData = {
        name: 'Khách hàng',
        email: 'no-email@ndtravel.com',
        phone: phone.trim(),
        content: 'Yêu cầu tư vấn tour qua số điện thoại'
      };
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/contact/submit`, {
        ...
        credentials: 'omit' // Không gửi cookies/session
      });
    ...
    <>
      {Object.entries(groupedDomesticDestinations).map(([region, provinces]) => (
        <div key={region} className="categories__item categories__item--expandable">
          ...
          >
            <span>{capitalizeFirstLetter(region)}</span>
            <ChevronDown
                ...
                return (
                  <div key={province} className="categories__submenu-item">
                    <div
                      ...
                      style={{ cursor: 'pointer' }}
                    >
                      <span>{capitalizeFirstLetter(province)}</span>
                    </div>
                  </div>
                );
              })}
      ...
      {Object.entries(groupedInternationalDestinations).map(([continent, countries]) => (
        <div key={continent} className="categories__item categories__item--expandable">
          <div
            ...
            style={{ cursor: 'pointer' }}
          >
            <span>{capitalizeFirstLetter(continent)}</span>
            <ChevronDown
  ...
  const renderSeasonalMenu = () => {
    ...
    let rawName = currentCategory.name;
    
    // Loại bỏ "Tour" ở đầu nếu có
    rawName = rawName.replace(/^Tour\s+/i, '').replace(/^tour\s+/i, '').trim();
    
    // Viết hoa chữ đầu
    seasonName = capitalizeWords(rawName);
    } else if (slug) {
        ...
        {/* Header */}
          <header className="tour-listing__header">
              <div className="tour-listing__title-wrapper">
                <h1 className="tour-listing__title">
                  {searchQuery 
                    ? `Kết quả tìm kiếm cho "${searchQuery}"` 
                    : (currentCategory?.pageTitle && currentCategory.pageTitle.trim()) || capitalizeFirstLetter(currentCategory?.name) || 'Danh sách tour'
                  }
                </h1>
              ...
              <TruncatedText
              text={searchQuery 
                ? `Tìm thấy các tour phù hợp với từ khóa "${searchQuery}"` 
                : (currentCategory?.pageSubtitle && currentCategory.pageSubtitle.trim()) || currentCategory?.description || 'Khám phá các hành trình hấp dẫn được ND Travel tuyển chọn dành riêng cho bạn.'
              }
              maxLength={160}
            ...
            <section className="tour-listing__tours">
              ...
              {tours.length === 0 && !loadingCategory ? (
                <div className="tour-listing__no-results">
                  <p>
                    {searchQuery 
                      ? `Không tìm thấy tour nào với từ khóa "${searchQuery}". Hãy thử tìm kiếm với từ khóa khác.`
                ...
                <>                
                  {tours.map((tour) => {
                    ...
                    // Ưu tiên sử dụng featuredImage, fallback về images[0], sau đó placeholder
                    const imageUrl = tour.featuredImage || 
                                  (tour.images && tour.images.length > 0 ? tour.images[0] : null) || 
                                  tour.image || 
                                  PLACEHOLDER_IMAGES.TOUR_CARD;
                    ...
                    return (
                      <article key={tour._id} className="tour-item">
                        <div className="tour-item__image">
                          ...
                          <Link to={`/tour/${tour.slug}`} state={{ categorySlug: currentCategory?.slug }}>
                            <img className="tour-item__img" src={imageUrl} alt={tour.title} />
                          </Link>
                        </div>

                        <div className="tour-item__content">
                          <Link to={`/tour/${tour.slug}`} state={{ categorySlug: currentCategory?.slug }} className="tour-item__title">
                            {tour.title}
                          </Link>
              ...
              <div className="contact__content">
                ...
                <div className="contact__form" onSubmit={handleSubmitPhone}>
                  ...
                  <div className="contact__form-wrapper">
                    <input
                      className={`contact__input ${phoneError ? 'error' : ''}`}
                      type="tel"
