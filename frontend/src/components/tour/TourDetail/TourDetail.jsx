...
import { formatTimeWithParentheses } from '../../../utils/timeFormatter';
...
const TourDetail = ({ tourData, isLoading = false, isError = false, error = null, suggestedTours = [] }) => {
  ...
  const gallery = useMemo(() => {
    ...
    // Ưu tiên ảnh đại diện làm ảnh đầu tiên
    if (tourData?.featuredImage) {
      galleryImages.push({
        id: 1,
        src: replacePlaceholderUrl(tourData.featuredImage),
    ...
    if (Array.isArray(tourData?.images) && tourData.images.length > 0) {
      tourData.images.forEach((rawUrl, i) => {
        galleryImages.push({
    ...
    // Nếu có ít nhất 1 ảnh, return gallery
    if (galleryImages.length > 0) {
      return galleryImages;
    }

    // Fallback: dùng ảnh chính cũ nếu có
    const fallback = [];
    if (tourData?.image) {
      fallback.push({
        id: 1,
    ...
    if (fallback.length === 0) {
      fallback.push({ 
        id: 1, 
        src: PLACEHOLDER_IMAGES.TOUR_GALLERY, 
        alt: title, 
        caption: title,
        isFeatured: false
      });
      fallback.push({ 
        id: 2, 
        src: PLACEHOLDER_IMAGES.TOUR_DETAIL, 
        alt: title, 
        caption: title,
        isFeatured: false
      });
  ...
  }, [tourData?.featuredImage, tourData?.images, tourData?.image, tourData?.title]);
  ...
  const handleBookingSubmit = async (orderData, recaptchaToken) => {
    try {
      ...
      if (orderData.paymentMethod === 'Ví điện tử MoMo') {
            ...
            navigate('/thank-you', {
              state: {
                orderInfo: {
                  ...
                  paymentMethod: result.order?.paymentMethod || orderData.paymentMethod,
                  items: result.order?.items || orderData.items,
        ...
        // Các phương thức thanh toán khác (tiền mặt)
        navigate('/thank-you', {
          state: {
            orderInfo: {
              orderId: result.order?.orderId || 'N/A',
 ...
 //Xây dựng một dải hình ảnh chính "vô hạn" bằng cách lặp lại
  ...
  const nextImage = useCallback(() => {
    ...
    // Nếu đang animate, dừng animation hiện tại và chuyển ngay
    ...
    // Sử dụng requestAnimationFrame để đảm bảo animation hoàn thành
          ...
          <div className={`tour-detail__booking-card ${isSidebarScrolled ? 'tour-detail__booking-card--scrolled' : ''}`}>
            ...
            <div className="tour-detail__pricing">
              {tourData?.originalPrice && 
               ...
               tourData?.originalPrice > tourData?.discountPrice && (
                <div className="tour-detail__price-original">{formatPrice(tourData.originalPrice)}</div>
              )}
              <div className="tour-detail__price-current">{formatPrice(tourData?.discountPrice || tourData?.price || 0)}</div>
            </div>
                      ...
                      (() => {
                        ...
                        const groupedByMonth = tourData.tourDetails
                          ...
                          .reduce((acc, detail) => {
                            ...
                            const time = formatTimeWithParentheses(detail.startTime);
                            ...
                            acc[monthKey].days.push(`${day}${time}`);
                            return acc;
                          }, {});
