...
const RetryPayment = () => {
    ...
    const handleBackToTour = async () => {
        try {
            // Lấy tourId từ orderData
            const tourId = orderData?.tourId || (orderData?.items && orderData.items[0]?.tourId);
            
            if (tourId) {
                // Gọi API để lấy slug từ tourId
                try {
                    const response = await axiosInstance.get(`/api/public/tours/${tourId}/slug`);
                    ...
                    if (response.data.success && response.data.data?.slug) {
                        ...
                        toast.info('Đang chuyển về trang tour');
                        return;
                    }
                } catch (error) {
                ...
                // Fallback: sử dụng tourId trực tiếp
            ...
            console.error('Error in handleBackToTour:', error);
