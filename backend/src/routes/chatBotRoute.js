...
const conversationContextController = require('../controllers/conversationContextController');
...
router.get('/tours/:tourId/schedule', chatbotController.getTourSchedule);

router.get('/tours/slug/:slug/schedule', chatbotController.getTourScheduleBySlug);
...
// ========== CONVERSATION CONTEXT & MEMORY ROUTES ==========
// ⚠️ IMPORTANT: Specific routes MUST come before parameterized routes

// Lấy session statistics (specific route)
router.get('/context/:sessionId/stats', conversationContextController.getSessionStats);

// Lấy short-term memory (specific route)
router.get('/context/:sessionId/memory', conversationContextController.getShortTermMemory);

// Lấy conversation context cho session (parameterized route)
router.get('/context/:sessionId', conversationContextController.getContext);

// Xóa conversation context
router.delete('/context/:sessionId', conversationContextController.clearContext);

// Ghi nhận tour view (specific route)
router.post('/preferences/:userIdentifier/tour-view', conversationContextController.recordTourView);

// Ghi nhận feedback (specific route)
router.post('/preferences/:userIdentifier/feedback', conversationContextController.recordFeedback);

// Lấy user preferences (parameterized route)
router.get('/preferences/:userIdentifier', conversationContextController.getUserPreferences);

// Cập nhật user preferences
router.put('/preferences/:userIdentifier', conversationContextController.updateUserPreferences);
