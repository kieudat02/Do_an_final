...
const { bookingRateLimit, orderLookupRateLimit } = require('../middleware/rateLimiter');
...
// Public Tour Detail API by slug (SEO-friendly)
router.get('/tours/detail/:slug', async (req, res) => {
    try {
        await tourController.getTourBySlug(req, res);
    } catch (error) {
        console.error('Error in public tour detail by slug API:', error);
        res.status(500).json({
            success: false,
...
// API để lấy slug từ ID (for redirect)
router.get('/tours/:id/slug', async (req, res) => {
    try {
        await tourController.getTourSlugById(req, res);
    } catch (error) {
        console.error('Error in get tour slug API:', error);
        res.status(500).json({
            success: false,
...
// Public Tour Detail API by ID (legacy support)
...
// API tăng views cho tour (public API)
router.post('/tours/:tourId/increment-views', async (req, res) => {
    try {
        await tourController.incrementViews(req, res);
    } catch (error) {
        console.error('Error in increment tour views API:', error);
        res.status(500).json({
            success: false,
...
router.post('/order/create', bookingRateLimit, verifyRecaptcha, async (req, res) => {
    try {
...
router.post('/order/send-otp', orderLookupRateLimit, verifyRecaptcha, async (req, res) => {
    try {
        const orderController = require('../controllers/orderController');
...
router.post('/order/lookup-with-otp', orderLookupRateLimit, async (req, res) => {
    try {
        const orderController = require('../controllers/orderController');
