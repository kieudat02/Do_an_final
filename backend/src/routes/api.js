...
const { chatbotRateLimit, syncHistoryRateLimit } = require('../middleware/rateLimiter');
const { validateMessage, validateSyncHistory, validateSessionId } = require('../middleware/inputValidator');
const { getMetrics } = require('../services/loggerService');
...
// Chat History Routes (no auth required for public chatbot)
router.post('/chatbot/sync-history', syncHistoryRateLimit, validateSyncHistory, chatBotController.syncChatHistory);
router.get('/chatbot/history/:sessionId', validateSessionId, chatBotController.getChatHistory);
router.get('/chatbot/sessions', chatBotController.getUserChatSessions);
router.delete('/chatbot/sessions/:sessionId', validateSessionId, chatBotController.deleteChatSession);
    ...
    try {
        ...
        res.json({
            ...
            metrics: {
                ...
                sensitiveBlocked: metrics.sensitiveQuestionsBlocked,
                outOfScopeBlocked: metrics.outOfScopeQuestionsBlocked,
...
router.get('/dashboard/monthly-revenue', homeController.apiMonthlyRevenue);
router.get('/dashboard/revenue-by-payment', homeController.apiRevenueByPaymentMethod);
router.get('/dashboard/top-tours', homeController.apiTopTours); 
router.get('/dashboard/payment-success-rates', homeController.apiPaymentSuccessRates);

// Cleanup stats endpoint
router.get('/cleanup/stats', homeController.apiCleanupStats);
...
router.put('/orders/:id/customer-info', checkPermission("UPDATE_ORDER"), orderController.updateCustomerInfo);

// Gửi lại email xác nhận đơn hàng
router.post('/orders/:id/resend-email', checkPermission("UPDATE_ORDER"), orderController.resendOrderEmail);
...
// Cập nhật quyền (chỉ Super Admin và Admin)
...
//------- ChatBot Routes --------
router.post('/chatbot/send-message', chatbotRateLimit, validateMessage, chatBotController.sendMessage);
router.post('/chatbot/create-session', chatBotController.createSession);
router.post('/chatbot/unanswered-questions', chatbotRateLimit, chatBotController.saveUnansweredQuestion);
router.post('/chatbot/send-otp-order-lookup', chatbotRateLimit, chatBotController.sendOTPForOrderLookup);
router.post('/chatbot/lookup-order-with-otp', chatbotRateLimit, chatBotController.lookupOrderWithOTP);
router.post('/chatbot/check-payment-status', chatbotRateLimit, chatBotController.checkPaymentStatus);
