...
const { createSafeSearchRegex } = require('../utils/regexUtils');

// Utility function để làm tròn giá đến hàng nghìn (đồng bộ với frontend)
const roundThousand = (value) => Math.round((value || 0) / 1000) * 1000;
...
exports.list = async (req, res) => {
    ...
    if (req.query.q) {
        const safeRegex = createSafeSearchRegex(req.query.q);
        if (safeRegex) filter.title = safeRegex;
    }

    // Date filters
    // Ngày đi: lọc theo TourDetail.dayStart trong ngày được chọn
    if (req.query.startDate) {
        try {
            const selected = new Date(req.query.startDate);
            const startOfDay = new Date(selected.getFullYear(), selected.getMonth(), selected.getDate(), 0, 0, 0, 0);
            const endOfDay = new Date(selected.getFullYear(), selected.getMonth(), selected.getDate() + 1, 0, 0, 0, 0);

            const matchedDetails = await TourDetail.find({
                dayStart: { $gte: startOfDay, $lt: endOfDay }
            }).select('tourId');

            const matchedTourIds = matchedDetails.map(d => d.tourId);
            // Nếu có kết quả thì chỉ lấy các tour này, nếu không có thì đảm bảo trả về rỗng
            filter._id = matchedTourIds.length > 0 ? { $in: matchedTourIds } : { $in: [] };
        } catch (e) {
            // Nếu lỗi parse ngày, bỏ qua bộ lọc ngày
        }
    }
...
exports.create = async (req, res) => {
    try {
        ...
        // Handle file uploads - req.files is an object with field names as keys
        if (req.files) {
            // Handle featured image (single file)
            if (req.files.featuredImage && req.files.featuredImage.length > 0) {
                featuredImage = req.files.featuredImage[0].path || "/uploads/" + req.files.featuredImage[0].filename;
                image = featuredImage; // Set as main image for backward compatibility
            }
        ...
        if (
            ...
            req.flash("message", "Thêm tour mới thành công!");
...
exports.update = async (req, res) => {
    try {
        ...
        if (req.files) {
            ...
            if (req.files.featuredImage && req.files.featuredImage.length > 0) {
                updateData.featuredImage = req.files.featuredImage[0].path || "/uploads/" + req.files.featuredImage[0].filename;
                updateData.image = updateData.featuredImage; // Update main image for backward compatibility
            }
        ...
        if (
            ...
            // Return redirect response for form submissions
            req.flash("message", "Cập nhật tour thành công!");

            // Build redirect URL with original query parameters
            let redirectUrl = "/tour";
            const queryParams = [];
            
            // Preserve important query parameters
            if (req.body.returnPage) queryParams.push(`page=${req.body.returnPage}`);
            if (req.body.returnLimit) queryParams.push(`limit=${req.body.returnLimit}`);
            if (req.body.returnStatus) queryParams.push(`status=${req.body.returnStatus}`);
            if (req.body.returnHighlight) queryParams.push(`highlight=${req.body.returnHighlight}`);
            if (req.body.returnCategory) queryParams.push(`category=${req.body.returnCategory}`);
            if (req.body.returnDeparture) queryParams.push(`departure=${req.body.returnDeparture}`);
            if (req.body.returnDestination) queryParams.push(`destination=${req.body.returnDestination}`);
            if (req.body.returnTransportation) queryParams.push(`transportation=${req.body.returnTransportation}`);
            if (req.body.returnQ) queryParams.push(`q=${encodeURIComponent(req.body.returnQ)}`);
            if (req.body.returnStartDate) queryParams.push(`startDate=${req.body.returnStartDate}`);
            if (req.body.returnEndDate) queryParams.push(`endDate=${req.body.returnEndDate}`);
            if (req.body.returnQuickFilter) queryParams.push(`quickFilter=${req.body.returnQuickFilter}`);
        ...
        const toursWithPricing = await Promise.all(
            tours.map(async (tour) => {
                ...
                const discountPrice = (isFinite(bestDiscountPrice) && bestDiscountPrice > 0)
                    ? roundThousand(bestDiscountPrice)
                    : (minPrice > 0 ? roundThousand(minPrice) : (tour.price || 0));
                const originalPrice = (bestOriginalPrice > 0)
                    ? roundThousand(bestOriginalPrice)
                    : (maxPrice > 0 ? roundThousand(maxPrice) : discountPrice);
...
// Lấy thông tin chi tiết tour theo slug API
exports.getTourBySlug = async (req, res) => {
    try {
        ...
        // Lịch trình xây dựng từ hành trình
        const schedule = (tour.itinerary || []).map(item => ({
            day: item.day,
            title: item.title,
            content: decodeHtmlEntities(item.details || '')
        ...
        // Tính toán giá cả tương tự như các method khác
        const minAvail = calculateMinPrice(tourDetails);
        const maxAvail = calculateMaxPrice(tourDetails);
        const fallbackMin = calculateMinPriceAll(tourDetails);
        const fallbackMax = calculateMaxPriceAll(tourDetails);
...
// Lấy slug từ ID (để redirect từ old URL)
exports.getTourSlugById = async (req, res) => {
    try {
        const tour = await Tour.findById(req.params.id).select('slug');
        ...
        console.error("Error in getTourSlugById:", error);
        res.status(500).json({
            success: false,
            message: "Lỗi server khi lấy slug tour",
            error: error.message,
        });
    }
};
            ...
            return res.status(200).json({
                ...
                data: {
                    ...
                    pricing: {
                        ...
                        // Tính giá sau giảm giá (làm tròn đến hàng nghìn)
                        finalAdultPrice: roundThousand(firstDetail.adultPrice * (100 - firstDetail.discount) / 100),
                        finalChildrenPrice: firstDetail.childrenPrice > 0 ? roundThousand(firstDetail.childrenPrice * (100 - firstDetail.discount) / 100) : 0,
                        finalChildPrice: firstDetail.childPrice > 0 ? roundThousand(firstDetail.childPrice * (100 - firstDetail.discount) / 100) : 0,
                        finalBabyPrice: firstDetail.babyPrice > 0 ? roundThousand(firstDetail.babyPrice * (100 - firstDetail.discount) / 100) : 0
                    },
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                pricing: {
                    ...
                    // Tính giá sau giảm giá (làm tròn đến hàng nghìn)
                    finalAdultPrice: roundThousand(targetDetail.adultPrice * (100 - targetDetail.discount) / 100),
                    finalChildrenPrice: targetDetail.childrenPrice > 0 ? roundThousand(targetDetail.childrenPrice * (100 - targetDetail.discount) / 100) : 0,
                    finalChildPrice: targetDetail.childPrice > 0 ? roundThousand(targetDetail.childPrice * (100 - targetDetail.discount) / 100) : 0,
                    finalBabyPrice: targetDetail.babyPrice > 0 ? roundThousand(targetDetail.babyPrice * (100 - targetDetail.discount) / 100) : 0
                },
                availableStock: targetDetail.stock,
                dayStart: targetDetail.dayStart,
                dayReturn: targetDetail.dayReturn,
                startTime: targetDetail.startTime,
...
// API để tăng views cho tour (public API)
exports.incrementViews = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: "Đã tăng views thành công",
            data: {
                tourId: tour._id,
                views: tour.views
            }
        ...
        res.status(500).json({
            success: false,
            message: "Lỗi server khi tăng views tour",
            error: error.message
        });
    }
};
