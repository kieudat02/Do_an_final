...
const { sendReviewThankYouEmail } = require('../utils/emailUtils');
...
exports.createReview = async (req, res) => {
    try {
        ...
        await review.populate('user', 'fullName avatar email');

        // Gửi email cảm ơn đánh giá (không chặn response)
        const user = await User.findById(userId);
        if (user && user.email) {
            // Tìm order để lấy orderId (nếu có)
            ...
            sendReviewThankYouEmail({
                ...
                orderId: userOrder ? userOrder.orderId : 'N/A',
                tourName: tour.title,
                rating: parseInt(rating),
                reviewContent: trimmedComment
            }).catch(error => {
