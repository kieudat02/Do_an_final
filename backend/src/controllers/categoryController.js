...
exports.showEditForm = async (req, res) => {
    try {
        ...
        res.render("category/edit", { category, csrfToken: req.csrfToken() });
    } catch (error) {
...
exports.update = async (req, res) => {
    try {
    const { name, description, status, pageTitle, pageSubtitle, sortOrder } = req.body;
...
exports.apiCreate = async (req, res) => {
    try {
    const { name, description, pageTitle, pageSubtitle, sortOrder } = req.body;
...
exports.updateSortOrder = async (req, res) => {
    try {
        ...
        const updatePromises = categories.map(({ id, sortOrder }) => 
            Category.findByIdAndUpdate(id, { sortOrder: parseInt(sortOrder) }, { new: true })
        );
        ...
        res.status(500).json({
            success: false,
            message: "Có lỗi xảy ra khi cập nhật thứ tự sắp xếp",
        });
    }
};
