...
function setupEditFormHandler() {
    ...
    editForm.addEventListener('submit', function(e) {
        ...
        fetch(editForm.action, {
            method: 'POST',
            headers: {
                'X-CSRF-Token': window.csrfToken || formData.get('_csrf') || ''
            },
            body: formData,
            credentials: 'include',
        ...
        .then(response => {
            if (response.type === 'opaqueredirect' || response.status === 302) {
                ...
                // Build return URL with preserved parameters
                let returnUrl = '/tour';
                const returnParams = [];
                
                // Get preserved parameters from hidden fields
                const returnPage = document.getElementById('returnPage')?.value;
                const returnLimit = document.getElementById('returnLimit')?.value;
                const returnStatus = document.getElementById('returnStatus')?.value;
                const returnHighlight = document.getElementById('returnHighlight')?.value;
                const returnCategory = document.getElementById('returnCategory')?.value;
                const returnDeparture = document.getElementById('returnDeparture')?.value;
                const returnDestination = document.getElementById('returnDestination')?.value;
                const returnTransportation = document.getElementById('returnTransportation')?.value;
                const returnQ = document.getElementById('returnQ')?.value;
                const returnStartDate = document.getElementById('returnStartDate')?.value;
                const returnEndDate = document.getElementById('returnEndDate')?.value;
                const returnQuickFilter = document.getElementById('returnQuickFilter')?.value;
                
                if (returnPage && returnPage !== '1') returnParams.push(`page=${returnPage}`);
                if (returnLimit) returnParams.push(`limit=${returnLimit}`);
                if (returnStatus) returnParams.push(`status=${returnStatus}`);
                if (returnHighlight) returnParams.push(`highlight=${returnHighlight}`);
                if (returnCategory) returnParams.push(`category=${returnCategory}`);
                if (returnDeparture) returnParams.push(`departure=${returnDeparture}`);
                if (returnDestination) returnParams.push(`destination=${returnDestination}`);
                if (returnTransportation) returnParams.push(`transportation=${returnTransportation}`);
                if (returnQ) returnParams.push(`q=${encodeURIComponent(returnQ)}`);
                if (returnStartDate) returnParams.push(`startDate=${returnStartDate}`);
                if (returnEndDate) returnParams.push(`endDate=${returnEndDate}`);
                if (returnQuickFilter) returnParams.push(`quickFilter=${returnQuickFilter}`);
...
function setupSearchHandler() {
    ...
    // Chỉ submit khi nhấn Enter trong ô tìm kiếm
...
// Bỏ auto submit theo debounce; chỉ cho phép Enter mới submit
const searchInputGlobal = document.querySelector('input[name="q"]');
...
function addPromotion() {
    ...
    newRow.innerHTML = `
        ...
        <div class="col-md-6">
            <textarea class="form-control" name="promo_desc[]" rows="2"
                      placeholder="Mô tả khuyến mãi"></textarea>
        </div>
        <div class="col-md-2">
            <button type="button" class="btn btn-outline-danger btn-sm"
...
function initializeTourListPage() {
    // Initialize filter state on page load (không auto-submit)
    const content = document.getElementById('advancedFilterContent');
    const icon = document.getElementById('filterToggleIcon');
    ...
    if (form) {
        const filterElements = form.querySelectorAll('select, input[type="date"]');
        // Chỉ highlight thay đổi, KHÔNG submit tự động
        filterElements.forEach(element => {
            element.addEventListener('change', function() {
                this.style.borderColor = '#007bff';
                setTimeout(() => {
    ...
    // Không auto submit khi gõ tìm kiếm
...
function initializeCKEditor() {
    ...
    // CKEditor configuration for pricing notes (simplified, no images)
    const pricingEditorConfig = {
        toolbar: [
            'bold', 'italic', '|',
            'bulletedList', 'numberedList', '|',
            'link', '|',
    ...
    // Initialize CKEditor for pricing fields (simplified, no custom image buttons)
    document.querySelectorAll('.ckeditor-pricing').forEach(function(element) {
        if (element.dataset.ckeditorInitialized) return;

        element.dataset.ckeditorInitialized = 'true';
        ClassicEditor
            .create(element, pricingEditorConfig)
            .then(editor => {
                window.tourEditors.pricing = window.tourEditors.pricing || [];
                window.tourEditors.pricing.push(editor);
                
                // Add resize handler but no custom image buttons
                addImageResizeHandler(editor);
            ...
            .catch(error => {
                console.error('Error initializing CKEditor for pricing:', error);
            });
    });
    ...
    function updateAllEditorsBeforeSubmit() {
        ...
        // Update pricing editors
        window.tourEditors.pricing.forEach(editor => {
            if (editor.sourceElement) {
                editor.sourceElement.value = getEditorDataWithImages(editor);
            }
        });
...
function setupImagePreview() {
    ...
    if (featuredImageInput) {
        featuredImageInput.addEventListener('change', function(e) {
            ...
            const previewContent = document.getElementById('featuredPreviewContainer');
            ...
            if (e.target.files.length > 0) {
                ...
                reader.onload = function(e) {
                    previewContent.innerHTML = `
                        <img src="${e.target.result}" alt="Ảnh đại diện preview" class="img-fluid w-100" style="height: 200px; object-fit: cover; border-radius: 4px;">
                    ...
                    reader.onload = function(e) {
                        ...
                        imagePreview.innerHTML = `
                            <div class="image-preview-item border rounded overflow-hidden">
                                <img src="${e.target.result}" alt="Ảnh bổ sung ${index + 1}" class="img-fluid w-100" style="height: 150px; object-fit: cover;">
                                <div class="text-center mt-1">
                                    <small class="text-muted">Ảnh bổ sung ${index + 1}</small>
                                </div>
                            </div>
                        `;
