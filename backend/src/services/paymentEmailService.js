...
class PaymentEmailService {
    ...
    // Gửi email hoàn tiền (deprecated - sử dụng sendOnlinePaymentCancelledAutoRefund thay thế)
    async sendOnlinePaymentRefund(orderData, refundReason = '') {
        try {
            const emailData = {
                customerEmail: orderData.email,
                customerName: orderData.customer,
    ...
    // Gửi email thông báo hủy tour với hoàn tiền
    async sendOnlinePaymentCancelledAutoRefund(orderData, cancellationReason = '') {
        try {
            const emailData = {
                customerEmail: orderData.email,
                customerName: orderData.customer,
            ...
            return await onlinePaymentEmailUtils.sendCancelledAutoRefundEmail(emailData);
        } catch (error) {
            throw error;
        }
    }
    ...
    async sendAutoRefundSuccess(orderData, refundData, cancellationReason = '') {
        try {
            const emailData = {
                customerEmail: orderData.email,
                customerName: orderData.customer,
            ...
            return await onlinePaymentEmailUtils.sendAutoRefundSuccessEmail(emailData);
        } catch (error) {
            throw error;
        }
    }
    ...
    async sendAutoRefundFailed(orderData, errorMessage, cancellationReason = '') {
        try {
            const emailData = {
                customerEmail: orderData.email,
                customerName: orderData.customer,
            ...
            return await onlinePaymentEmailUtils.sendAutoRefundFailedEmail(emailData);
        } catch (error) {
            throw error;
        }
    }
    ...
    async sendOnlinePaymentConfirmed(orderData) {
        try {
            const firstItem = orderData.items && orderData.items[0];
            ...
            return await onlinePaymentEmailUtils.sendOrderConfirmationEmail(emailData);
        } catch (error) {
            throw error;
        }
    }
    ...
    // Gửi email đã xác nhận đơn (confirmed - chưa thanh toán)
    ...
    // Gửi email đã thanh toán tiền mặt thành công (payment status = completed)
    async sendCashPaymentReceived(orderData) {
        try {
            const emailData = this.prepareCashEmailData(orderData);
            return await cashPaymentEmailUtils.sendPaymentConfirmedEmail(emailData);
        } catch (error) {
            throw error;
        }
    }

    // Gửi email xác nhận VÀ thanh toán tiền mặt cùng lúc
    async sendCashConfirmedAndPaid(orderData) {
        try {
            const emailData = this.prepareCashEmailData(orderData);
            return await cashPaymentEmailUtils.sendConfirmedAndPaidEmail(emailData);
        } catch (error) {
            throw error;
        }
    }
    ...
    async handleOnlinePaymentEmail(orderData, status, additionalData) {
        switch (status) {
            ...
            // case 'refund' - deprecated: sử dụng 'cancelled_auto_refund' thay thế
